...
| SafeWallpaperBridgeController | src/SafeWallpaperBridgeController.hpp + .cpp | `bridge` property; Q_INVOKABLE setLoaded / pushUserProperties / pushGeneralProperties forwarding to the bridge. | QML-only: the real type is never registered on the WebChannel, which is why the bridge's own properties can stay READ-only. |
...
| WebProfileRegistry  | src/WebProfileRegistry.hpp + src/WebProfileRegistry.cpp | Static: storageNameFor(workshopId). Q_INVOKABLEs: profileFor(workshopId), interceptorFor(workshopId), liveProfileCount(). | Every WebProfileRegistry {} element forwards to WebProfileRegistryStore, a `pragma Singleton` QtObject with no C++ counterpart (it exists only to give the stub's per-instance facades one shared table, the way the real class shares one process-wide table). The store's cache and call log live in a `.pragma library` JS module (WebProfileRegistryData.js), not as QML properties -- a property read inside profileFor() would make Qt's dependency tracker attribute it to whichever OTHER binding called profileFor(), so an unrelated lookup could re-trigger an already-settled one. `callsSnapshot()` / `resetCalls()` expose the call log (every workshopId passed to profileFor()) for tests asserting which ids a swap looked up. |
