...
if [[ "$MODE" == "coverage" ]]; then
    ...
    # Object set: read from the coverage_objects.txt that tests/CMakeLists.txt
    # writes at configure time, one binary path per line.  Hand-typing the
    # object list here would diverge from that file the moment a test target
    # is added, and llvm-cov treats a missing object as absent rather than as
    # an error, so the report would silently stop covering it.
    cov_manifest="$cov_build/coverage_objects.txt"
    [[ -s "$cov_manifest" ]] || fail "coverage_objects.txt not generated by the coverage configure ($cov_manifest)"
    mapfile -t cov_bins < "$cov_manifest"
    cov_objs="${cov_bins[0]}"
    for cov_bin in "${cov_bins[@]:1}"; do
        cov_objs="$cov_objs -object=$cov_bin"
    done
